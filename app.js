...
app.get('/city/:cityId/genres', async (req, res) => {
    ...
    try {
        const query = 'SELECT DISTINCT g.name, g.genre_id FROM genre g JOIN items i ON g.genre_id = i.id_genre WHERE i.id_city = $1';
...
app.get('/city/:cityId/genre/:genreId/subgenre/:subgenreId/quantities', async (req, res) => {
    ...
    try {
        const query = 'SELECT DISTINCT q.value FROM items i INNER JOIN quantity q ON i.id_quantity = q.quantity_id WHERE i.id_city = $1 AND i.id_genre = $2 AND i.id_subgenre = $3';
